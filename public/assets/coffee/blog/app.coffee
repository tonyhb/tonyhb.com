...
PostContentView = require('blog/views/postItemView')
...
blog = {
  ...
  Controller: M.Controller.extend
    ...
    showPost: (url) ->
      ...
      return if @_findAndShowPost(url)
    ...
    _findAndShowPost: (slug) ->
      @_region = @_region || Radio.channel('regions').request('content')
      post = blog.collection.findWhere({Slug: slug})
      @_region.show new PostContentView({ model: post }) if post
      return post?
}
...
Radio.channel('router').on 'post', (slug) ->
  blog.controller.showPost(slug)
  blog.router.navigate slug

Radio.channel('router').on 'home', ->
